...
brange(Max,Max,[]) :- !.
brange(Min,Max,R) :- Min2 is Min + 1, R = [Min2| T], brange(Min2, Max,T).
...
get_sim_notes3(Poslist, Notelist) :- maplist(get_sim_notes2, Poslist, Y), flatten(Y,Notelist),!.
...
generate_measure_examples :- setof(X,barline(X),Xlist2), append([0],Xlist2,Xlist), map2neighbours(Xlist, brange, Ylist),maplist(get_sim_notes3, Ylist, Ylist2), maplist(assert_example(measure), Ylist2), !.
