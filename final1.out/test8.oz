local
   ...
   fun {MakeVoice Pitches Duration}
      {Score.makeScore2 seq(items:
			       {Map Pitches fun {$ P}
				  note(pitch: P duration: {FD.int 1})
					    end }
			    startTime: 0
			    duration:Duration
			   ) unit }
   ...
   in
                  Solution = {Score.makeScore sim(items:[{MakeVoice {FD.list NP 60#100} NP} {MakeVoice {FD.list NP 60#72} NP} {MakeVoice {FD.list NP 60#72} NP} {MakeVoice {FD.list NP 60#72} NP} ]  timeUnit:beats) unit}
      \insert 'oz_loops.oz'
   end
